...
serve(async (req) => {
  ...
  try {
    ...
    // ✅ Kunin ang target value (true para i-on, false para i-off)
    const enable = body.enable === true;

    if (!email || !otp) {
      return json(origin, 400, { success: false, message: "Missing required fields" });
    }
    ...
    if (profErr || !prof) return json(origin, 404, { success: false, message: "professor not found" });

    const professor_id = prof.id;
    const role = "professor";
    const purpose = "enable_2fa"; // Mas generic na purpose name

    // ✅ Get latest OTP row
    const { data: row, error: rowErr } = await admin
      .from("password_reset_otps") // Gamit ang existing OTP table mo
      .select("*")
    ...
    if (rowErr || !row) return json(origin, 400, { success: false, message: "No valid OTP request found" });
    if (row.used_at) return json(origin, 400, { success: false, message: "OTP already used" });

    const now = new Date();
    if (now > new Date(row.expires_at)) return json(origin, 400, { success: false, message: "OTP expired" });

    if (Number(row.attempts ?? 0) >= 5) return json(origin, 429, { success: false, message: "Too many attempts" });

    // ✅ Hash check
    const expected = String(row.otp_hash ?? "");
    const got = await sha256(`${professor_id}|${email}|${otp}`);

    if (got !== expected) {
      await admin.from("password_reset_otps").update({ attempts: (row.attempts ?? 0) + 1 }).eq("id", row.id);
      return json(origin, 400, { success: false, message: "Invalid OTP" });
    }

    // ✅ UPDATE DATABASE: I-on o I-off ang 2FA depende sa pinasa ng Flutter
    const { error: upErr } = await admin
      .from("professors")
      .update({ two_fa_enabled: enable })
      .eq("id", professor_id);

    if (upErr) return json(origin, 500, { success: false, message: "Failed to update 2FA status" });
